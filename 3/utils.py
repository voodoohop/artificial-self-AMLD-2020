...
def read_conversation_data(data_path):
    ...
    if data_path is None:
        ...
        if len(input_files) == 0:
            ...
            raise Exception(
                f"Multiple files found in {data_folder}. Specify file with chatistics_data_path argument."
            )
...
def get_grouped_conversation_data(
    data_path, use_cache=True, cache_path="grouped_conversation_data.json"
):
    ...
    num_interactions = 0
    for conversation_name, g in tqdm(
        df.groupby("conversationWithName"),
        total=len(df["conversationWithName"].unique()),
        position=0,
    ):
        ...
        if len(g["senderName"].unique()) == 2 and len(g) > 10:
            ...
            for i, row in g.iterrows():
                ...
                if (
                    timestamp
                    > timedelta(hours=new_conversation_delay_hours) + time_last_message
                ):
            ...
            if len(conversation) > min_num_interactions_per_conversation:
                ...
                num_interactions += len(conversation)
    logger.info(
        f"Generated {len(data.keys()):,} conversations with a total of {num_interactions:,} interactions..."
    )
...
def get_input_task3(
    ...
    cached_path = (
        f"cached_input_task3_{tokenizer.__module__}_{num_candidates}_{seed}.pkl"
    )
    if use_cache:
        ...
        for conversation in conversations:
            ...
            for interaction in conversation:
                ...
                if interaction["senderType"] == "person1":
                    ...
                    history_tk = [
                        tokenizer.encode(h) for h in history[-(2 * max_history + 1) :]
                    ]
                    ...
                    for j, candidate in enumerate(candidates_tk):
                        lm_labels = bool(
                            j == num_candidates - 1
                        )  # the last candidate is the correct reply
                        # build training example (num_candidates x input_length)
                        instance = build_input_from_segments(
                            history_tk, candidate, tokenizer, lm_labels=lm_labels
                        )
                        ...
                        dataset["mc_labels"].append(
                            num_candidates - 1
                        )  # index of true_answer
    ...
    logger.info(
        f"Generated a total of {num_examples:,} training examples from {len(conv_data):,} conversations consisting of {num_interactions:,} interactions and {num_messages:,} messages..."
    )
...
def build_input_from_segments(
    history, reply, tokenizer, lm_labels=False, with_eos=True
):
