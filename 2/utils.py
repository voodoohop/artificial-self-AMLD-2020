...
def generate_input_task2(
    data_path, speaker1_tag="<speaker1>", speaker2_tag="<speaker2>"
):
    ...
    for conversation_name, g in tqdm(
        ...
        if (
            len(g["senderName"].unique()) == 2
            and len(g) > min_num_interactions_per_conversation
        ):
            # sort by time
    ...
    # write output data
    logger.info(
        f"Writing input file with {num_interactions:,} interactions to {CACHE_PATH}..."
    )
...
def get_input_task2(
    data_path, speaker1_tag="<speaker1>", speaker2_tag="<speaker2>", use_cache=True
):
    """Load input data for task 2"""
    if not os.path.isfile(CACHE_PATH) or not use_cache:
        generate_input_task2(
            data_path, speaker1_tag=speaker1_tag, speaker2_tag=speaker2_tag
        )
